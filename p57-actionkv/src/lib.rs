...
impl ActionKV {
    pub fn open(path: &Path) -> io::Result<Self> {
        // An example of the "Builder" pattern. Each method returns a new instance of the OpenOptions struct with the
        // relevant option set.
        let f = OpenOptions::new()
            // Enable reading
            .read(true)
            // Enable writing (not strictly necessary, as it's implied by append)
            .write(true)
            // Create a file at `path` if ir doesn't already exist
            .create(true)
